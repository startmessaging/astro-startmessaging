...
- Self-serve sign-up with one identity or business-registration document.
- Wallet-funded — no contract, no minimums.
- Test before fully activating with sandbox.



StartMessaging keeps customer-side paperwork to a single onboarding step:
        upload either a company registration document or a personal government
        ID. No template-by-template approval queue, no per-operator PE-ID
        registration.
