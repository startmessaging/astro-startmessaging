...
featured: false
image: "/blog/images/flash-call-verification-india-vs-sms-otp-developer-review.webp"
---
...
Voice OTP is the simplest to understand: the system calls the user, the call connects, and an automated voice reads out the verification code. The user listens to the code and types it into the app. This is an IVR-based flow â€” the call must be answered, the user must listen, and the experience is manual. Voice OTP is already [covered in a separate post](/blog/voice-otp-vs-sms-otp-india) and is a different product entirely from flash call.

### Missed Call Verification

Missed call verification flips the direction. The user calls a designated phone number (often a toll-free or local number). The system detects the incoming call, identifies the caller's number from the CLI, and confirms that the number is real and active. No code is exchanged â€” the call itself is the verification. This method is popular for lead generation and number confirmation flows where you need to verify that a phone number exists, not that the user possesses a secret.

### Flash Call / CLI Verification

Flash call is the most technically sophisticated of the three. The system places a call to the user's mobile number and disconnects it before the phone fully rings. The verification "code" is embedded in the Caller Line Identification (CLI) â€” specifically, the last 4â€“6 digits of the incoming number. For Android users, an SDK reads the call log, extracts those digits, and auto-submits them to your backend. The user sees nothing â€” the entire verification completes in 3â€“5 seconds without any interaction.

### Why the Distinction Matters

Each method has a different cost structure: voice OTP costs â‚¹0.30â€“â‚¹0.80 per call (full IVR connection). Missed call verification costs â‚¹0.01â€“â‚¹0.05 per call (toll-free incoming). Flash call costs â‚¹0.03â€“â‚¹0.08 per attempt (brief outgoing call). Each has different regulatory treatment under TRAI. And each requires different SDK capabilities on the client side. Confusing them leads to wrong cost projections and wrong compliance assumptions.
...
3. **The provider allocates a temporary outbound number** whose last 4â€“6 digits are the verification code. The provider tells your backend what these digits are.
4. **The provider places a call** from that temporary number to the user's mobile. The call is designed to drop within 1â€“3 seconds â€” before the user can answer, and in many cases before the phone rings audibly.
...
The total elapsed time is 3â€“5 seconds. The user did not type anything, did not switch apps, and may not have noticed the call at all. When it works, the experience is genuinely seamless.
...
**Google Play policy (2026):** Google restricts `READ_CALL_LOG` to apps where call log access is a core feature. You must submit a declaration explaining why your app needs this permission, and Google reviews it. Phone verification is an accepted use case, but expect the review cycle to add 1â€“2 weeks to your Play Store approval process. Some developers have reported rejections on first submission, requiring resubmission with a more detailed justification.
...
- **Display the incoming number** and ask the user to manually enter the last 4â€“6 digits. This defeats the "zero-touch" advantage.
- **Fall back to SMS OTP** automatically for all iOS users. This is the practical choice.

Calculate what percentage of your Indian users are on iOS. Nationally, iOS is roughly 4â€“5% of the smartphone market. In metro cities and among higher-income demographics, it can be 15â€“25%. Your SMS fallback volume scales directly with your iOS user share.

### BSNL Reality

CLI delivery on BSNL is inconsistent. BSNL's network infrastructure does not always propagate the full caller ID for brief calls, particularly calls that disconnect before the first ring completes. In testing, flash call verification on BSNL subscribers shows a 60â€“70% success rate compared to 90%+ on Jio and Airtel. Plan for 100% SMS fallback for BSNL users.
...
|---|---|---|
| SMS OTP (market range) | â‚¹0.12â€“â‚¹0.25 | DLT-compliant, universal reach |
| SMS OTP (StartMessaging) | â‚¹0.25 | [No DLT registration required](/dlt-free-otp), rate limiting included |
| Flash call verification | â‚¹0.03â€“â‚¹0.08 | Operator-dependent, Android-only auto-verify |
| Voice OTP (IVR) | â‚¹0.30â€“â‚¹0.80 | Full call connection required |
...
- **iOS users (4â€“5% nationally, up to 25% in metro fintech apps):** These must receive SMS OTP. Every iOS verification adds an SMS cost on top of your flash call infrastructure.
- **BSNL users (approximately 8â€“10% of mobile subscribers):** Low flash call success rate means most BSNL verifications become SMS fallbacks.
- **Failed flash calls (network congestion, DND settings, Android skin variations):** Roughly 5â€“10% of flash call attempts on supported networks fail and fall back to SMS.

A realistic cost model for 1,00,000 monthly verifications:

| Scenario | Flash call + SMS fallback | SMS OTP only (StartMessaging) |
|---|---|---|
| Flash call attempts (75% of volume) | 75,000 Ã— â‚¹0.05 = â‚¹3,750 | â€” |
| SMS fallback (25% of volume) | 25,000 Ã— â‚¹0.25 = â‚¹6,250 | â€” |
| SMS OTP (100% of volume) | â€” | 1,00,000 Ã— â‚¹0.25 = â‚¹25,000 |
| **Total monthly cost** | **â‚¹10,000** | **â‚¹25,000** |
| **Monthly savings** | **â‚¹15,000** | â€” |
...
- **Play Store permission review cycles:** Initial review adds 1â€“2 weeks. Policy changes can require re-review.
- **Fallback logic engineering:** Building and maintaining the flash call â†’ SMS fallback pipeline is a non-trivial engineering investment. Testing across Jio, Airtel, Vi, and BSNL with different Android versions and OEM skins is an ongoing cost.
...
At what monthly volume does the flash call integration effort pay off? Assume the integration takes a senior developer 2â€“3 weeks (cost: â‚¹1.5â€“â‚¹3 lakh including testing). Ongoing maintenance is approximately 2â€“3 days per quarter.

| Monthly volume | Monthly savings | Months to break even (â‚¹2 lakh integration cost) |
|---|---|---|
| 10,000 verifications | â‚¹1,500 | 133 months (not worth it) |
| 50,000 verifications | â‚¹7,500 | 27 months |
| 1,00,000 verifications | â‚¹15,000 | 13 months |
| 5,00,000 verifications | â‚¹75,000 | 3 months |
...
Honest answer: no. TRAI's DLT framework governs commercial SMS â€” specifically, messages sent through telecom operators' SMS infrastructure. Flash calls are telephone calls, not SMS messages. They fall under a different regulatory domain (telephony, not commercial communications). There is no DLT registration requirement for placing a phone call and disconnecting it.

This is genuinely an advantage for developers who find [DLT registration](/blog/what-is-dlt-registration-india) burdensome. But it is also the reason regulators have been wary â€” flash call sidesteps the compliance framework that TRAI built specifically to control commercial messaging.
...
- **Startups with cost-sensitive unit economics.** When you are verifying lakhs of users monthly and SMS costs are a meaningful line item in your P&L, the 50â€“70% per-verification cost reduction matters.
...
The fallback order â€” Flash Call â†’ SMS OTP â†’ WhatsApp OTP â€” optimises for cost on the happy path and reach on the fallback paths. Flash call handles the cheapest verifications for eligible users. SMS OTP via [StartMessaging](/otp-api) catches everyone else with universal reach at â‚¹0.25 per message. WhatsApp OTP serves as the last resort for edge cases where both flash call and SMS fail.
